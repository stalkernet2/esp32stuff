...
// Под вопросом - по WiFi, возможно, имеются ограничения, рекомендуют использовать только 32, 33, 34, 35, 36, 39. Проверить работоспособность 25, 26, 27, 32, 33, 34, 35, 36. Однако, в любом случае, следует использовать bluetooth подключение, WiFi коннект - вариант не лучший(WiFi подключение может быть только одно). ОДНАКО, для Bluetooth нужно приложения для телефона, а для WiFi достаточно захостить сайт на самой esp32
